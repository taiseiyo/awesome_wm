...
local ICON_DIR = os.getenv("HOME") .. '/.config/awesome/widget/brightness-widget/'
local handle = io.popen("brightnessctl -d intel_backlight max")
local max_brightness = handle:read("*a")
handle:close()
...
local function worker(user_args)
    ...
    if program == 'brightnessctl' then
       get_brightness_cmd = 'brightnessctl -d intel_backlight get'
       inc_brightness_cmd = 'brightnessctl -d intel_backlight set +10%'
       dec_brightness_cmd = 'brightnessctl -d intel_backlight set 10%-'
    else
    ...
    if type == 'icon_and_text' then
        brightness_widget.widget = wibox.widget {
	       ...
	       level = (level / max_brightness) * 100
	       self:get_children_by_id('txt')[1]:set_text(math.floor(level) .. '%')	       
            end
        }
    else
